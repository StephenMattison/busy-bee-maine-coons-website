...
#### G.1 Cloudflare Challenge Platform tanks Best Practices + TBT (mandatory checklist)

Live Lighthouse often shows **only** these as remaining blockers after the site itself is clean:

| Symptom | Source URL | Cause |
|---------|------------|--------|
| Best Practices **Uses deprecated APIs** | `/cdn-cgi/challenge-platform/scripts/jsd/main.js` | CF **JavaScript Detections** / Bot Fight challenge bootstrap (Shared Storage, Protected Audience, etc.) |
| Performance **Total Blocking Time** / long main-thread work | same `jsd/main.js` | Challenge script evaluation (often multi-second in lab) |
| Small “cache lifetimes” / unused JS tips | `/cdn-cgi/…` | Not in the repo — ignore or fix in CF dashboard |

**Repo cannot fix `jsd/main.js`.** To reach **100 Best Practices** and clean TBT on Cloudflare:
